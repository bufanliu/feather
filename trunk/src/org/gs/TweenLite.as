...
package org.gs
{
	...
	import org.gs.plugins.*;
	import org.gs.utils.tween.*;
		...
		public function complete($skipRender:Boolean=false):void
		{
			...
			if (_hasPlugins)
			{
				for (var i:int = this.tweens.length - 1; i > -1; i--)
				{
					...
					{ //function calls are expensive performance-wise, so don't call the plugin's onComplete() unless necessary. Most plugins don't require them.
